...
export const useRoomListStore = create((set, get)=>{
    return {
        ...
        addMember : async (roomID, username) => {
            ...
            const {curRoom, addMember}= useCurRoomStore.getState()
            for(let i = 0; i < roomList.length; i++) {
                if(roomID == roomList[i].roomID) {
                    const member = (await getUser(username)).data
                    if(curRoom.roomID == roomID) addMember(member)
                    else roomList[i].memberList.push(member)
                    return
                }
            }
        },
        ...
        deleteMember : (roomID, username) => {
            const roomList = get().roomList
            const {curRoom, deleteMember}= useCurRoomStore.getState()
            for(let i = 0; i < roomList.length; i++) {
                if(roomID == roomList[i].roomID) {
                    if(curRoom.roomID == roomID) deleteMember(username)
                    else {
